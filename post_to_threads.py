...
def generate_post():
    ...
    if is_promo:
        prompt_text = """ヘッドミント京都祇園店で働くセラピストとして、Threadsに投稿する文を1つ書いてください。

テーマ:「""" + theme + """」

【バズる投稿にするための条件】
・1行目は悩みを抱えた人が「これ私のこと？」と思う共感フックで始める
・「70分3,980円」を自然な流れで必ず盛り込む
・押しつけがましくなく、背中をそっと押すような柔らかい言い方で来店を促す
・「〜でお困りではないですか？」「〜な方にこそ来てほしい」など読者に寄り添う表現
・最後は「ぜひ一度試してみてください」より「気になったらコメントください」等コメントを促す
・文ごとに改行、各文に絵文字、ハッシュタグなし、120文字以内
・「完全個室」「ヘッドセラピー」は絶対に使わない
・時間帯（夜に・夕方・朝など）の表現は使わない
・絵文字は20代前半の女性セラピストが使うような可愛い系・共感系を使う（🩷🫶🥹🌸✨🍃🫧☺️💗🤍🫰など）🌿💡🎧💭のような硬い絵文字は使わない
・「。」「、」は使わない（改行や「！」「〜」で自然につなぐ）
・投稿文のみ出力（説明や前置きは不要）"""
    else:
        prompt_text = """ヘッドミント京都祇園店で働くセラピストとして、Threadsに投稿する文を1つ書いてください。
...
【バズる投稿にするための条件】
・1行目は読者が「わかる」「気になる」と感じる共感フックで始める
・実際の施術現場のリアルなエピソードや気づきを盛り込む
・「〜ですよね？」「〜な人いませんか？」など読者に語りかける表現を使う
・最後はコメントしたくなる問いかけか、思わず保存したくなる気づきで締める
・セラピストの人柄が伝わる温かみのある口調
・文ごとに改行、各文に絵文字、ハッシュタグなし、120文字以内
・「完全個室」「ヘッドセラピー」は絶対に使わない
・時間帯（夜に・夕方・朝など）の表現は使わない
・絵文字は20代前半の女性セラピストが使うような可愛い系・共感系を使う（🩷🫶🥹🌸✨🍃🫧☺️💗🤍🫰など）🌿💡🎧💭のような硬い絵文字は使わない
