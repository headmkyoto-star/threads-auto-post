...
・追加する一言だけ出力（書き出しは繰り返さない）"""
        ...
        season = get_season()
        prompt_text = f"""京都祇園のドライヘッドスパ店で働くセラピストとして、Threadsの投稿を1つ書いて。
...
・全体で80文字以内
・スマホでぱっと打ったような荒削りな口語
・毎回構成を変える（例：本音をつぶやく、驚きを伝える、共感を求める、愚痴っぽく言う、など）
・最後はコメントしたくなる問いかけで終わる
・各文の末尾に絵文字を1つつける（😮‍💨🙌😅🫠🤔💆👀💪🥺など自然な絵文字）
・🩷💗🌸🫶などネカマ絵文字は使わない
・通常「？→絵文字」、感情強め「絵文字→？」
・「。」「、」は使わない
・時間・季節・情景描写・サロン雰囲気の描写はしない
・きれいにまとめない。「素敵な投稿」を作ろうとしない
・関西弁は無理に使わない。自然に出る時だけ（めっちゃ、〜へん程度）
・「完全個室」「ヘッドセラピー」は使わない
・ハッシュタグなし
・文と文の間は改行
・投稿文だけ出力"""
