...
def generate_post():
    ...
    if is_promo:
        prompt_text = "ヘッドミント京都祇園店のセラピストとしてThreadsに投稿する文を1つ書いてください。テーマ:「" + theme + "」ルール:投稿文のみ出力。文ごとに改行。各文に絵文字。ハッシュタグなし。150文字以内。価格は必ず「70分3,980円」と明記すること。「完全個室」「ヘッドセラピー」とは絶対に書かないこと。時間帯（夜に・夕方・朝など）の表現は使わないこと。"
    else:
        prompt_text = "ヘッドミント京都祇園店のセラピストとしてThreadsに投稿する文を1つ書いてください。テーマ:「" + theme + "」ルール:投稿文のみ出力。文ごとに改行。各文に絵文字。ハッシュタグなし。150文字以内。「完全個室」「ヘッドセラピー」とは絶対に書かないこと。時間帯（夜に・夕方・朝など）の表現は使わないこと。"
