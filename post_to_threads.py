...
・時間に関する表現は一切使わない
・「完全個室」「ヘッドセラピー」は使わない
・「あなた」という表現は使わない（「またあなたに来たい」などNG。「また来たい」「またお願いしたい」のように自然に言い換える）
...
・日記系：その日の施術で感じたこと・うれしかったこと・気づいたことをさらっと書く。お客さんの個人情報は出さない（他のスタイルより少し多めに使う）

【最重要ルール・必ず守ること】
・絶対に各文の末尾に絵文字をつける。1文でも絵文字なしの行を作らない
・「。」「、」は絶対に使わない。句読点を一切入れない
・「」（鍵括弧）は絶対に使わない。引用・強調でも「」は使わない
